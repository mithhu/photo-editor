...
import type { FaceKeypoint, MakeupSettings, MakeupItem } from '../types'
...
function drawRegion(ctx: CanvasRenderingContext2D, keypoints: FaceKeypoint[], indices: number[]): void {
...
function applyLipstick(canvas: HTMLCanvasElement | OffscreenCanvas, keypoints: FaceKeypoint[], color: string, opacity: number): void {
  if (opacity <= 0) return
  ...
  const octx = overlay.getContext('2d') as OffscreenCanvasRenderingContext2D

  octx.fillStyle = `rgba(${r},${g},${b},1)`
  drawRegion(octx as unknown as CanvasRenderingContext2D, keypoints, FACE_REGIONS.lips)

  const feathered = new OffscreenCanvas(canvas.width, canvas.height)
  const fctx = feathered.getContext('2d') as OffscreenCanvasRenderingContext2D
...
function applyBlush(canvas: HTMLCanvasElement | OffscreenCanvas, keypoints: FaceKeypoint[], color: string, opacity: number): void {
  if (opacity <= 0) return
  ...
  octx.fillStyle = `rgba(${r},${g},${b},1)`
  drawRegion(octx as unknown as CanvasRenderingContext2D, keypoints, FACE_REGIONS.leftCheek)
  drawRegion(octx as unknown as CanvasRenderingContext2D, keypoints, FACE_REGIONS.rightCheek)
...
function applyEyeliner(canvas: HTMLCanvasElement | OffscreenCanvas, keypoints: FaceKeypoint[], color: string, opacity: number): void {
  if (opacity <= 0) return
  ...
  for (const eyeRegion of [FACE_REGIONS.leftEye, FACE_REGIONS.rightEye]) {
    const pts = eyeRegion.slice(0, Math.ceil(eyeRegion.length / 2))
      .map((i: number) => keypoints[i]).filter(Boolean)
...
function applyEyeshadow(canvas: HTMLCanvasElement | OffscreenCanvas, keypoints: FaceKeypoint[], color: string, opacity: number): void {
  if (opacity <= 0) return
  ...
  for (const [eyeRegion, browRegion] of [
    ...
    const eyePts = eyeRegion.slice(0, Math.ceil(eyeRegion.length / 2))
      .map((i: number) => keypoints[i]).filter(Boolean)
    const browPts = browRegion.map((i: number) => keypoints[i]).filter(Boolean)
 ...
 */
export function applyMakeupToCanvas(canvas: HTMLCanvasElement | OffscreenCanvas, keypoints: FaceKeypoint[], settings: MakeupSettings): void {
  const {
...
export const MAKEUP_PRESETS: Record<string, MakeupPreset> = {
  natural: {
    name: 'Natural',
