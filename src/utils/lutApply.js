 ...
 * @param {{ size: number, type: '1d'|'3d', data: Float32Array, domainMin: number[], domainMax: number[] }} lut
 */
...
function applyLUT1D(d, size, data, domainMin, domainMax) {
...
function applyLUT3D(d, size, data, domainMin, domainMax) {
  const rangeR = domainMax[0] - domainMin[0]
  const rangeG = domainMax[1] - domainMin[1]
  const rangeB = domainMax[2] - domainMin[2]
  const maxIdx = size - 1
