...
let _programs: Record<string, WebGLProgram> = {}
let _uniforms: Record<string, Record<string, WebGLUniformLocation | null>> = {}
let _sourceTexture: WebGLTexture | null = null
let _fbos: (WebGLFramebuffer | null)[] = [null, null]
let _fboTextures: (WebGLTexture | null)[] = [null, null]
let _curvesTexture: WebGLTexture | null = null
let _quadBuf: WebGLBuffer | null = null
let _lastW: number = 0, _lastH: number = 0
...
function ensureFBOs(gl: WebGLRenderingContext, w: number, h: number): void {
...
function drawQuad(gl: WebGLRenderingContext): void {
  gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4)
}

function activateProgram(gl: WebGLRenderingContext, name: string): Record<string, WebGLUniformLocation | null> {
...
export function initWebGL(): boolean {
  if (_gl) return true
  try {
    ...
    const shaderPairs: Record<string, string> = {
      color: FRAG_COLOR,
...
function buildCurveLUT(points: [number, number][]): Uint8Array {
  ...
  const xs: number[] = sorted.map(p => p[0])
  const ys: number[] = sorted.map(p => p[1])
  ...
  for (let px = 0; px < 256; px++) {
    ...
    const dx: number = xs[seg + 1] - xs[seg] || 1
    const tt: number = (t - xs[seg]) / dx
    const a: number = (2 * tt * tt * tt - 3 * tt * tt + 1) * ys[seg]
    const b2: number = (tt * tt * tt - 2 * tt * tt + tt) * dx * m[seg]
    const cc: number = (-2 * tt * tt * tt + 3 * tt * tt) * ys[seg + 1]
    const d: number = (tt * tt * tt - tt * tt) * dx * m[seg + 1]
    lut[px] = Math.min(255, Math.max(0, Math.round((a + b2 + cc + d) * 255)))
  }
  return lut
}

function uploadCurvesLUT(gl: WebGLRenderingContext, curves: CurvePoints): void {
  const rgbLUT: Uint8Array = buildCurveLUT(curves.rgb)
  const rLUT: Uint8Array = buildCurveLUT(curves.red)
  const gLUT: Uint8Array = buildCurveLUT(curves.green)
  const bLUT: Uint8Array = buildCurveLUT(curves.blue)
...
const FILTER_TYPE_MAP: Record<string, number> = { brightness: 0, contrast: 1, saturate: 2, sepia: 3, grayscale: 4, 'hue-rotate': 5 }

// ─── Render params interface ─────────────────────────────────────────
 ...
 */
export function renderWithWebGL(sourceCanvas: HTMLCanvasElement, params: WebGLRenderParams): HTMLCanvasElement | null {
  if (!_gl) { if (!initWebGL()) return null }
  const gl = _gl!

  const w: number = sourceCanvas.width
  const h: number = sourceCanvas.height
  _canvas!.width = w
  _canvas!.height = h
  gl.viewport(0, 0, w, h)
  ...
  let readTex: WebGLTexture | null = _sourceTexture
  let pingPong: number = 0

  function renderPass(programName: string, setupFn: (u: Record<string, WebGLUniformLocation | null>) => void): void {
  ...
  renderPass('color', (u) => {
    ...
    const ops: FilterOp[] = filterOps || []
    gl.uniform1i(u['u_filterCount'], Math.min(ops.length, 6))
    for (let i = 0; i < 6; i++) {
      const op: FilterOp | undefined = ops[i]
      gl.uniform1i(u[`u_filterTypes[${i}]`], op ? (FILTER_TYPE_MAP[op.type] ?? 0) : 0)
      gl.uniform1f(u[`u_filterValues[${i}]`], op ? op.value : 0)
    }

    const hslRanges: string[] = ['red', 'orange', 'yellow', 'green', 'cyan', 'blue', 'purple', 'magenta']
    for (let i = 0; i < 8; i++) {
      const ch: HSLChannel = hsl?.[hslRanges[i]] || { h: 0, s: 0, l: 0 }
      gl.uniform1f(u[`u_hslH[${i}]`], ch.h)
      gl.uniform1f(u[`u_hslS[${i}]`], ch.s)
      gl.uniform1f(u[`u_hslL[${i}]`], ch.l)
    }

    const hasCurves: boolean = !!(curves && Object.entries(curves).some(([, pts]) =>
    ...
    const st: Partial<SplitTone> = splitTone || {}
    ...
    const filmId: number = filmEmulation ? (FILM_ID_MAP[filmEmulation] ?? -1) : -1
    ...
    const hasCM: boolean = !!(cm && (cm.red?.g !== 0 || cm.red?.b !== 0 || cm.green?.r !== 0 || cm.green?.b !== 0 || cm.blue?.r !== 0 || cm.blue?.g !== 0 ||
    ...
    const hasGM: boolean = !!gm?.enabled
  ...
  const ts: Partial<TiltShift> = tiltShift || {}
  const tsBlur: number = ts.blur || 0
  const hasPost: boolean = vignette > 0 || tsBlur > 0 || ((grain?.amount ?? 0) > 0) || filmGrain > 0

  const sharpTex: WebGLTexture | null = readTex
  let blurredTex: WebGLTexture | null = readTex

  if (tsBlur > 0) {
    const blurRadius: number = Math.min(tsBlur, 20)
    renderPass('blurH', (u) => {
      gl.uniform1i(u['u_image'], 0)
      gl.uniform2f(u['u_texelSize'], 1.0 / w, 1.0 / h)
