...
let _cache: PreviewCache = { src: null, previews: null }
 ...
 */
function loadThumbImageData(imageSrc: string): Promise<ThumbData> {
 ...
 */
export async function generateFilterPreviews(imageSrc: string): Promise<Record<string, string>> {
  if (!imageSrc) return {}
  ...
  for (const preset of FILTER_PRESETS) {
    if ((preset as any).ops.length === 0) {
      ...
      applyPixelFilters(copy, (preset as any).ops)
