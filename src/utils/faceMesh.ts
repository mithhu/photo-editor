...
interface FaceDetector {
  estimateFaces(input: HTMLCanvasElement | HTMLImageElement): Promise<DetectedFace[]>
}
 ...
 */
export async function loadFaceMesh(): Promise<FaceDetector | null> {
  if (detector) return detector
  if (loadFailed) return null

  if (loadingPromise) return loadingPromise

  loadingPromise = (async (): Promise<FaceDetector | null> => {
    try {
...
export async function detectFaceLandmarks(
  ...
  try {
    ...
    return await det.estimateFaces(input as HTMLCanvasElement | HTMLImageElement)
  } catch (err) {
    console.warn('[FaceMesh] Detection failed:', (err as Error).message)
    return []
  }
}
 ...
 */
export function isFaceMeshLoading(): boolean {
