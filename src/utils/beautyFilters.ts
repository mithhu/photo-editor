...
import type { FaceKeypoint, BeautySettings } from '../types'
...
function createMultiRegionMask(
  keypoints: FaceKeypoint[],
  regionArrays: number[][],
  width: number,
  height: number,
  ...
  if (feather > 0) {
    ...
    const bctx = blurred.getContext('2d', { willReadFrequently: true }) as OffscreenCanvasRenderingContext2D
...
function createSkinMask(
  ...
  const fctx = feathered.getContext('2d', { willReadFrequently: true }) as OffscreenCanvasRenderingContext2D
...
function applySkinToneEvenness(
  imageData: ImageData,
  keypoints: FaceKeypoint[],
  amount: number,
  skinMask: Float32Array | null
