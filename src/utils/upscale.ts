...
const MAX_INPUT_PIXELS: number = 1024 * 1024 // 1 megapixel max input → 2 megapixel output

async function getUpscaler(onProgress?: (msg: string) => void): Promise<any> {
...
function downscaleToLimit(img: HTMLImageElement): HTMLImageElement | HTMLCanvasElement {
  const { naturalWidth: w, naturalHeight: h } = img
  const pixels: number = w * h
  if (pixels <= MAX_INPUT_PIXELS) return img

  const scale: number = Math.sqrt(MAX_INPUT_PIXELS / pixels)
  const nw: number = Math.round(w * scale)
  const nh: number = Math.round(h * scale)
...
export async function upscaleImage(
  ...
  const inputW: number = input instanceof HTMLCanvasElement ? input.width : ow
  const inputH: number = input instanceof HTMLCanvasElement ? input.height : oh
