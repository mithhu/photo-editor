...
import { applyPixelFilters, type PixelFilterOp } from '../utils/pixelFilters'
...
export function BatchProcessor({ editState, onBack }: BatchProcessorProps): React.JSX.Element {
  ...
  const handleFileSelect = useCallback((e: ChangeEvent<HTMLInputElement>): void => {
    ...
    const loadPromises = imageFiles.map(f => new Promise<BatchFile>((resolve) => {
      const reader = new FileReader()
      reader.onload = () => resolve({ name: f.name, src: reader.result as string })
  ...
  const processAll = useCallback(async (): Promise<void> => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        processed.push({ name: files[i].name, error: err instanceof Error ? err.message : 'Unknown error' })
      }
    }
...
function buildFilterOps(editState: EditState): FilterOp[] {
...
async function processImage(imageSrc: string, editState: EditState): Promise<string> {
  const img = new Image()
  img.src = imageSrc
  ...
  if (needsPixelPass) {
    ...
    if (filterOps.length > 0) {
      applyPixelFilters(imageData, filterOps as PixelFilterOp[])
    }
