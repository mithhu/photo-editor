...
import type { EditState, GradientMap } from '../types'
...
interface GradientMapPanelProps {
  editState: EditState
  applyChange: (updater: ((prev: EditState) => EditState) | Partial<EditState>) => void
}

export function GradientMapPanel({ editState, applyChange }: GradientMapPanelProps) {
  const gm: GradientMap = editState.gradientMap || { enabled: false, shadows: '#1a1a2e', highlights: '#e8d5b7', intensity: 0.7 }
  const [activePreset, setActivePreset] = useState<string | null>(null)

  const update = (patch: Partial<GradientMap>): void => {
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 flex-1">
              ...
              <input
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => { setActivePreset(null); update({ shadows: e.target.value }) }}
            ...
            <label className="flex items-center gap-2 flex-1">
              ...
              <input
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => { setActivePreset(null); update({ highlights: e.target.value }) }}
                className="w-8 h-6 rounded cursor-pointer border border-zinc-700 bg-transparent"
              />
          ...
          <label className="flex items-center gap-3">
            ...
            <input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => update({ intensity: Number(e.target.value) / 100 })}
