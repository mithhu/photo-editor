...
function createRadialMask(): Mask {
  return {
    id: Date.now(),
    type: 'radial',
    centerX: 0.5,
...
function createLinearMask(): Mask {
  return {
    id: Date.now(),
    type: 'linear',
    centerX: 0.5,
...
export function MaskPanel({ masks = [], onMasksChange }: MaskPanelProps) {
  const addRadial = (): void => {
    if (masks.length >= MAX_MASKS) return
    onMasksChange([...(masks || []), createRadialMask()])
  }

  const addLinear = (): void => {
    if (masks.length >= MAX_MASKS) return
    onMasksChange([...(masks || []), createLinearMask()])
  }

  const updateMask = (index: number, updates: Partial<Mask>): void => {
    const next = [...(masks || [])]
    next[index] = { ...next[index], ...updates }
    onMasksChange(next)
  }

  const deleteMask = (index: number): void => {
          ...
          >
            ...
            <label className="flex items-center gap-2 text-sm text-zinc-400">
              <input
                type="checkbox"
                checked={!!mask.invert}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateMask(index, { invert: e.target.checked })}
