...
function getAspectRatio(cropRatio: string): number | null {
...
export function CropOverlay({ cropRegion, onCropChange, isActive, cropRatio }: CropOverlayProps) {
  const overlayRef = useRef<HTMLDivElement>(null)
  const dragRef = useRef<DragState | null>(null)
  const [isDragging, setIsDragging] = useState<boolean>(false)
  const [dragType, setDragType] = useState<'move' | 'resize' | null>(null)

  const clamp = (v: number, lo: number = 0, hi: number = 1): number => Math.min(hi, Math.max(lo, v))

  const handlePointerDown = useCallback((e: React.PointerEvent<HTMLDivElement>, handleId: string | null) => {
    e.stopPropagation()
    e.preventDefault()
    ...
    const type: 'move' | 'resize' = handleId ? 'resize' : 'move'
    dragRef.current = {
      type,
      handle: handleId,
      startX: e.clientX,
