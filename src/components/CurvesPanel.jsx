...
const HIT_RADIUS = 10
const DEFAULT_POINTS = [[0, 0], [1, 1]]

function drawCurveCanvas(ctx, points, channelColor) {
  const w = CANVAS_SIZE
  const h = CANVAS_SIZE
...
export function CurvesPanel({ curves, onChange }) {
  ...
  const channelColor = CHANNELS.find((c) => c.id === activeChannel)?.color ?? '#ffffff'
  ...
  const handleMouseMove = useCallback(
    (e) => {
      ...
      const nx = Math.max(0, Math.min(1, pos.x / CANVAS_SIZE))
      const ny = Math.max(0, Math.min(1, 1 - pos.y / CANVAS_SIZE))
      const updated = sorted.map((p, i) => (i === draggingIdx ? [nx, ny] : p))
