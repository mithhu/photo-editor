...
export function EditorHeader({ onUndo, onRedo, canUndo, canRedo, showCompare, onToggleCompare, onAutoEnhance, onNewImage, onDownload, onShare, onBatch, onResetAll, onOpenShortcuts, canvasRef }: EditorHeaderProps): React.JSX.Element {
  ...
  const handleNativeShare = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (err instanceof Error && err.name !== 'AbortError') onShare?.()
    } finally {
      setSharing(false)
    }
  }, [canvasRef, onShare])
