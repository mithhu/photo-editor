...
const COLLAGE_LAYOUTS = [
  ...
  { id: 'left-right', name: '1+2', custom: true, cells: 3 },
  { id: 'top-bottom', name: '2+1', custom: true, cells: 3 },
]
...
async function drawCustomLayout(ctx, layoutId, images, canvasW, canvasH, gap, radius) {
...
export function CollageBuilder({ onComplete, onBack }) {
  const [layout, setLayout] = useState(COLLAGE_LAYOUTS[3])
  ...
  const handleCellClick = useCallback((cellIndex) => {
    ...
    input.onchange = (e) => {
      ...
      reader.onload = () => {
        setImages((prev) => ({ ...prev, [cellIndex]: reader.result }))
      }
  ...
  const renderCell = useCallback(
    (index) => {
      ...
      return (
        <div
          className="w-full h-full border-2 border-dashed border-zinc-600 flex items-center justify-center text-zinc-500 text-2xl hover:border-amber-500 hover:text-amber-500 transition-colors"
          style={{ borderRadius: `${radius}px` }}
        >
          +
        </div>
  ...
  const renderLayoutThumbnail = (l) => {
    const isActive = layout.id === l.id
    if (l.custom) {
      if (l.id === 'left-right') {
        return (
          <div
            className={`grid h-10 rounded border-2 transition-colors ${isActive ? 'border-amber-500' : 'border-zinc-600 hover:border-zinc-500'}`}
    ...
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b border-zinc-800 shrink-0">
        <div className="flex items-center gap-3">
          <button
            onClick={onBack}
            className="flex items-center gap-1 text-zinc-400 hover:text-amber-400 transition-colors text-sm"
          >
        ...
        <button
          ...
          className="px-5 py-2 bg-amber-500 hover:bg-amber-400 disabled:bg-zinc-700 disabled:text-zinc-500 text-zinc-900 font-semibold rounded-lg text-sm transition-colors"
        >
