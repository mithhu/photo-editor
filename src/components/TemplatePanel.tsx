...
interface BuiltInTemplate {
  ...
  category: string
  preview: string
  apply: (applyChange: (updater: ((prev: EditState) => EditState) | Partial<EditState>) => void) => void
}
...
export function TemplatePanel({ applyChange, editState }: TemplatePanelProps) {
  ...
  const applyCustomTemplate = (template: CustomTemplate): void => {
    setSelectedTemplateId(template.id)
    applyChange((s) => ({ ...s, ...template.state }))
  }

  const handleApplyTemplate = (template: BuiltInTemplate): void => {
    setSelectedTemplateId(template.id)
