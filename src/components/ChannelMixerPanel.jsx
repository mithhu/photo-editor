...
export function ChannelMixerPanel({ editState, applyChange }) {
  const cm = editState.channelMixer || {
    red: { r: 100, g: 0, b: 0 },
    green: { r: 0, g: 100, b: 0 },
    blue: { r: 0, g: 0, b: 100 },
  }

  const update = (channel, source, value) => {
    applyChange({
      channelMixer: {
        ...cm,
              ...
              {['r', 'g', 'b'].map((src) => (
                <label key={src} className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) => update(ch, src, Number(e.target.value))}
