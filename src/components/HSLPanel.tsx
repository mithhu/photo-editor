...
interface HSLColorDef {
  id: string
  label: string
  color: string
}

const HSL_COLORS: HSLColorDef[] = [
...
interface HSLPanelProps {
  hsl: Record<string, HSLChannel> | undefined
  onChange: (color: string, channel: keyof HSLChannel, value: number) => void
}

export function HSLPanel({ hsl, onChange }: HSLPanelProps) {
  const [activeColor, setActiveColor] = useState<string>('red')
  const active: HSLChannel = hsl?.[activeColor] ?? { h: 0, s: 0, l: 0 }
